...
public class GalileoOSNMAConstellation extends Constellation{

    private final static boolean LOGGING_ENABLED = false;
    private final static char satType = 'E';
    protected static final String NAME = "Galileo E1 OSNMA";
    private static final String TAG = "GalileoE1OSNMAConstellation";
    ...
    public ArrayList<String> validatedSatsArray = new ArrayList<>();
...
//    private long timeRx;
    ...
    public GalileoOSNMAConstellation() {
        ...
        validatedSatsArray = new ArrayList<>();
    }
            ...
            for (GnssMeasurement measurement : event.getMeasurements()) {
                ...
                */
                ...
                if (towDecoded || towKnown) {
                    ...
                    //if(isSatValidated(measurement.getSvid()+"")){
            ...
            for (SatelliteParameters observedSatellite : observedSatellites) {
                ...
                */
                ...
                if (rnp == null) {
                    ...
                    System.out.println("Failed getting ephemeris data");
                    continue;
                }

                /*String satId = observedSatellite.getSatId()+"";
                boolean satValidated = MainActivity.isSatValidated(satId);
                if(satValidated){
                    excludedSatellites.add(observedSatellite);
                    System.out.println("Satellite "+observedSatellite.getSatId()+" NOT OSNMA VERIFIED");
                    continue;
                }else{
                    System.out.println(">>>>>>>>>>>> VADO CON "+satId+ " satelliti ignorati: "+excludedSatellites.size());
                }*/
    ...
    public static void registerClass() {
        register(
                NAME,
                GalileoOSNMAConstellation.class);
    }

    public void addSatToValidated(String svid){
        if(!isSatValidated(svid)){
            validatedSatsArray.add(svid);
        }
    }

    public void removeSatFromValidated(String svid){
        for(int i=0; i<validatedSatsArray.size(); i++){
            String[] satIdentifier = validatedSatsArray.get(i).split("-");
            if(satIdentifier[0].equalsIgnoreCase(svid)){
                validatedSatsArray.remove(i);
                break;
            }
        }
    }

    public boolean isSatValidated(String svid){
        boolean found = false;
        for(int i=0; i<validatedSatsArray.size(); i++){
            String[] satIdentifier = validatedSatsArray.get(i).split("-");
            if(satIdentifier[0].equalsIgnoreCase(svid)){
    ...
    public ArrayList<?> getValidatedSats(){
        return validatedSatsArray;
    }
