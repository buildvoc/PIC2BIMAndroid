...
public class TaskFulfillActivity extends BaseActivity {
    ...
    private void fillPhotoFlipper() {
        ...
        for (Photo photo : photoList.getPhotos()) {
            try {
                Bitmap bitmap = photo.getRotatedBitmap(6);
    ...
    private void addPhotoFromSnap(long photoId) {
        try {
            ...
            addPhotoToFlipper(photo.getRotatedBitmap(1));
            /* DEBUGCOM
            try {
                photo.toJSONObject();
            }catch (Exception e) {
    ...
    private void updatePhotoStats() {
        ...
        if (photoList.getPhotos().size() > 0) {
            ...
            if(photo.getProvider() != null){
                findViewById(R.id.tf_netstat_layout).setVisibility(View.VISIBLE);
                TextView netstat = findViewById(R.id.tf_textView_netstat_value);
                ...
                findViewById(R.id.tf_netstat_layout).setVisibility(View.GONE);
            }

            TextView osnmaValidated = findViewById(R.id.tf_textView_osnmaValidated_value);
    ...
    public void checkPendingValidations(View view){

        if(!Util.isInternetAvailable() ){
            MyAlertDialog.Builder builder = new MyAlertDialog.Builder(this);
            MyAlertDialog myAlertDialog = builder.build();
            myAlertDialog.setTitle(getString(R.string.tf_nointernet));
            myAlertDialog.setMessage(getString(R.string.tf_nointernetupload));
        ...
        boolean pendingValidations = false;
        for (Photo photo : photoList.getPhotos()) {
            if(photo.isOsnmaEnabled() && !photo.isSent() && photo.getProvider().equals(MainService.OFFLINE_LM_PROVIDER) && !photo.isOsnmaValidated()){
        ...
        if(pendingValidations){
            ...
            myAlertDialog.setMessage(getString(R.string.tf_pendingValidations));
            ...
            myAlertDialog.setButton(AlertDialog.BUTTON_POSITIVE, getString(R.string.dl_OK), (dialog, which) -> {
                ...
                AtomicInteger validationsRun = new AtomicInteger();
                int validationsToRun = photoList.getPhotos().size();
                for (Photo photo : photoList.getPhotos()) {
                    OfflineValidation.delayedValidation(photo, validatedSats -> {
                        validationsRun.getAndIncrement();
                        if (validatedSats > 0) {
                        ...
                        if(validationsRun.get() == validationsToRun){
