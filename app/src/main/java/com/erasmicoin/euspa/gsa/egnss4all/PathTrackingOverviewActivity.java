...
import com.erasmicoin.euspa.gsa.egnss4all.model.KMLHelper;
...
public class PathTrackingOverviewActivity extends BaseActivity {
    ...
    class PathTrackingAdapter extends RecyclerView.Adapter<PathTrackingAdapter.PathTrackingHolder> {
        ...
        class PathTrackingHolder extends RecyclerView.ViewHolder {
            ...
            public PathTrackingHolder(@NonNull View itemView) {
                ...
                exportKml = itemView.findViewById(R.id.pt_export_track);
            }
        }
        ...
        public void onBindViewHolder(@NonNull PathTrackingHolder holder, int position) {
            ...
            holder.exportKml.setOnClickListener(v -> {
                writeKML(ptPath);
            });
        }
    ...
    private void uploadAllPaths(List<PTPath> ptPaths) {
        ...
        for (PTPath ptPath : ptPaths) {
            ptPath.upload(MS.getAppDatabase(), MS.getRequestor(), uploadReceiver, this);
        }
    ...
    private ActivityResultLauncher<Intent> writeKMLActivityResultLauncher = registerForActivityResult(
    ...
    private PTPath pathToWrite;
    private void callKMLHelper(Uri uri){
        try {
            ParcelFileDescriptor pfd = getContentResolver().
                    openFileDescriptor(uri, "w");
            if (pfd != null) {
                KMLHelper.writeKML(pathToWrite, pfd);
                pfd.close();
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void writeKML(PTPath ptPath){

        pathToWrite = PTPath.createFromAppDatabase(MS.getAppDatabase(), ptPath.getAutoId());

        String filename = ptPath.getName()+".kml";
        ...
        writeKMLActivityResultLauncher.launch(intent);
    }
