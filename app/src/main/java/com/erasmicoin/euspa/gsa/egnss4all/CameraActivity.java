...
import com.erasmicoin.euspa.gsa.egnss4all.model.component.VerticalSeekBarAPI26;
...
public class CameraActivity extends BaseActivity implements CameraXConfig.Provider {
    ...
    public void serviceInit() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.HIGH_SAMPLING_RATE_SENSORS);
        }

        cameraViewModel = new ViewModelProvider(this).get(CameraViewModel.class);
        try {
            cameraViewModel.init(MS, this);
    ...
    private void updateLocationInfo() {
        ...
        TextView visiSatsTextView = findViewById(R.id.ca_textView_visisats);
        TextView usiSatsTextView = findViewById(R.id.ca_textView_usisats);

        findViewById(R.id.ca_layout_accuracy).setVisibility(View.VISIBLE);
        findViewById(R.id.ca_layout_usisats).setVisibility(View.GONE);
        findViewById(R.id.ca_layout_visisats).setVisibility(View.GONE);

        if(MS.getProvider().equals(MainService.ONLINE_LM_PROVIDER) && PersistData.getOSNMAValidated(this)){
            ((LinearLayout)findViewById(R.id.ca_layout_valisats)).setVisibility(View.VISIBLE);
        }
        ...
        if (lastLocation != null && lastLocation.getTime() + MAX_INFO_LOCATION_AGE_MILS > DateTime.now().toDate().getTime()) {
           /* latitudeTextView.setText(String.valueOf(lastLocation.getLatitude()));
            ...
            BigDecimal lat = new BigDecimal(String.valueOf(lastLocation.getLatitude())).setScale(7, BigDecimal.ROUND_HALF_UP);
            BigDecimal lon = new BigDecimal(String.valueOf(lastLocation.getLongitude())).setScale(7, BigDecimal.ROUND_HALF_UP);
            latitudeTextView.setText(lat.toString());
            longitudeTextView.setText(lon.toString());
            ...
            if(MS.getProvider().equals(MainService.ONLINE_LM_PROVIDER)){
                if(PersistData.getOSNMAValidated(this)){
                    ((LinearLayout)findViewById(R.id.ca_layout_valisats)).setVisibility(View.VISIBLE);
                    int validSats = lastLocation.getExtras().getInt("valisats");
                    TextView valiSatsTV = findViewById(R.id.ca_textView_valisats);
                    valiSatsTV.setText(validSats+"");
                }
            ...
            visiSatsTextView.setText(unavailable);
            usiSatsTextView.setText(unavailable);
            TextView valiSatsTV = findViewById(R.id.ca_textView_valisats);
            valiSatsTV.setText(unavailable);
        }
    ...
    private boolean checkData() {
        ...
        boolean locationsCorrect = cameraViewModel.photoDataController.isLocationCorrect(MS.getProvider());
        if (!locationsCorrect) {
            if (PersistData.getOSNMAValidated(this)){
                if(MS.getProvider().equals(MainService.ONLINE_LM_PROVIDER)){
                    addWarning(WARNING_MSG.NO_DATA_LOCATION, getString(R.string.ca_unverifiedPosition));
                }else{
                    addWarning(WARNING_MSG.NO_DATA_LOCATION, getString(R.string.ca_offlinePosition));
                }
    ...
    private void sendFile() {
        synchronized (isTakingPhoto) {
            try {
                ...
                //photo.setInavMessages(OfflineValidation.updateInavMessages(photo.getInavMessages(),MS.getSatValidationMap()));
