...
import static com.erasmicoin.euspa.gsa.egnss4all.model.locationManager.LMManager.INAV_MESSAGES_GATHERING;
...
import com.erasmicoin.euspa.gsa.egnss4all.model.gnss.NMEAParserApp;
...
public class PhotoDataController {
    ...
    public static final CharSequence INAV_SEPARATOR = "   ";
    ...
    public boolean isLocationCorrect(String provider) {
        synchronized (locations) {
            if (locations.size() < 2) {
                return false;
            }
            ...
            if(locations.size() > 0 && INAV_MESSAGES_GATHERING && provider.equals(MainService.OFFLINE_LM_PROVIDER)){
                if(inavMessages != null && inavMessages.size() < LMManager.MIN_INAV_MESSAGE_COUNT){
                    return false;
                }else if(inavMessages == null){
                    return false;
                }
            }
    ...
    public void setInavMessages(ArrayList<InavMessage> inavMessages) {
        this.inavMessages = inavMessages;
    }
