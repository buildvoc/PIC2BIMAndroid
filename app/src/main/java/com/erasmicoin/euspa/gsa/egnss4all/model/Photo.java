...
import com.erasmicoin.euspa.gsa.egnss4all.model.ekf.EkfController;
import com.erasmicoin.euspa.gsa.egnss4all.model.ekf.EkfData;
...
public class Photo {
    ...
    private boolean osnmaValidated;

    private boolean osnmaEnabled;

    private int validatedSats;

    private String inavMessages;
    ...
    public static Photo createFromResponse(JSONObject jsonObject, Long realId, String taskId, int indx, Context context, AppDatabase appDatabase) throws JSONException, IOException {
        ...
        if(jsonObject.has("provider") && !jsonObject.isNull("provider")){
            photo.setProvider(jsonObject.getString("provider"));
        }
        if(jsonObject.has("osnma_enabled") && !jsonObject.isNull("osnma_enabled")){
            photo.setOsnmaEnabled(jsonObject.getInt("osnma_enabled") == 0 ? false : true);
        }

        if(jsonObject.has("osnma_validated") && !jsonObject.isNull("osnma_validated")){
            photo.setOsnmaValidated(jsonObject.getInt("osnma_validated") == 0 ? false : true);
        }
        if(jsonObject.has("validated_sats") && !jsonObject.isNull("validated_sats")){
            photo.setValidatedSats(jsonObject.getInt("validated_sats"));
        }
    ...
    public static Photo createFromPhotoDataController(String taskId, PhotoDataController photoDataController, DateTime createdDateTime, String photoPathTemp, Context context, AppDatabase appDatabase) throws IOException, JSONException {
        ...
        if(photo.inavMessages == null && inavJsonArray.length() > 0){
            photo.inavMessages = inavJsonArray.toString();
        }
        photo.provider = photoDataController.getCurrentProvider();
        photo.taskId = taskId;
        String userId = LoggedUser.createFromAppDatabase(appDatabase).getId();
        photo.userId = userId;
        photo.context = context;
        ...
        if(photo.osnmaEnabled){
            if(!photo.provider.equals(MainService.OFFLINE_LM_PROVIDER)){
                ...
                photo.osnmaValidated = false;
                photo.validatedSats = 0;
            }
        }else{
            photo.osnmaValidated = false;
        }
        /*if(photoLocation.getExtras().containsKey("isOsnma")){
    ...
    public Bitmap getRotatedBitmap(int scale) throws IOException {
        ...
        if(rotation != 0){
            ...
            if(scale != 1)
                resizedBitmap = Bitmap.createScaledBitmap(rotatedBitmap,rotatedBitmap.getWidth()/scale, rotatedBitmap.getHeight()/scale, false);
            else
            ...
            if(scale != 1)
                resizedBitmap = Bitmap.createScaledBitmap(bitmap,bitmap.getWidth()/scale, bitmap.getHeight()/scale, false);
            else
                resizedBitmap = bitmap.copy(bitmap.getConfig(), false);
        }
    ...
    public org.json.simple.JSONObject toJSONObject() throws JSONException, Exception {
        ...
        jsonObject.put("osnmaEnabled", (osnmaEnabled ? 1 : 0));
        jsonObject.put("osnmaValidated", (osnmaValidated ? 1 : 0));
        jsonObject.put("validatedSats", validatedSats);

        jsonObject.put("provider", provider);
        //JSON UPLOAD INAVMESSAGES

//        jsonObject.put("inavMessages", inavMessages);
        jsonObject.put("inavMessages", "");
    ...
    public boolean isOsnmaValidated() {
        return osnmaValidated;
    }

    public void setOsnmaValidated(boolean osnmaValidated) {
        this.osnmaValidated = osnmaValidated;
    }

    public int getValidatedSats() {
        return validatedSats;
    }

    public void setValidatedSats(int validatedSats) {
        this.validatedSats = validatedSats;
    }

    public String getInavMessages(){ return inavMessages; }

    public void setInavMessages(String inavMessages){ this.inavMessages = inavMessages; }
    ...
    public boolean isOsnmaEnabled() {
        return osnmaEnabled;
    }

    public void setOsnmaEnabled(boolean osnmaEnabled) {
        this.osnmaEnabled = osnmaEnabled;
    }
